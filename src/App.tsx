...
export default function App() {
  ...
  const [taskRiskFilter, setTaskRiskFilter] = useState<'All' | 'High' | 'Medium' | 'Low'>('All');
          ...
          <div className="flex-1 min-h-0">
            ...
            {activeTab === 'tasks' && (
              <div className="space-y-8">
                ...
                <div className="bg-slate-900/40 border border-slate-800 p-4 rounded-2xl shadow-lg flex flex-col md:flex-row items-center gap-4 justify-between mt-6">
                  <div className="relative w-full md:w-2/3">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2 bg-slate-950 rounded-xl border border-slate-800 text-slate-100 placeholder:text-slate-650 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 text-sm font-medium"
                    />
                  </div>
                  <div className="flex items-center gap-2 w-full md:w-auto shrink-0 justify-end">
                    <span className="text-xs font-bold text-slate-400">Risk Level:</span>
                    <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800 gap-1">
                      {(['All', 'High', 'Medium', 'Low'] as const).map((r) => (
                        <button
                          key={r}
                          type="button"
                          onClick={() => setTaskRiskFilter(r)}
                ...
                <div className="space-y-6 mt-6">
                  {(() => {
                    ...
                    return (
                      <>
                        <h3 className="text-sm font-extrabold text-slate-400 uppercase tracking-widest flex justify-between items-center">
                          <span>Priority Queue ({filteredTasks.length} of {tasks.length})</span>
                          {(taskSearchQuery || taskRiskFilter !== 'All') && (
                            <button
                              onClick={() => { setTaskSearchQuery(''); setTaskRiskFilter('All'); }}
                              className="text-xs text-indigo-400 hover:text-indigo-300 font-bold underline cursor-pointer normal-case"
                            >
                              Clear Filters
                            </button>
                          )}
                        ...
                        {isLoadingTasks ? (
                          ...
                          <div className="bg-slate-900/60 border border-slate-800 p-10 rounded-2xl text-center shadow-xl">
                            <p className="text-slate-400 text-sm font-semibold">No tasks match your search query or selected risk filter level.</p>
                            <button
                              onClick={() => { setTaskSearchQuery(''); setTaskRiskFilter('All'); }}
                              className="mt-3 text-indigo-400 hover:text-indigo-300 text-xs font-bold underline cursor-pointer"
                            >
